...
let to_string relation_delim elt_delim m map_fold key_to_string data_to_string = 
  let stringlist = map_fold
    (fun k d list -> ((key_to_string k)^relation_delim^(data_to_string d))::list)
    m []
  in
  Mstring.concat_list elt_delim stringlist ""
...
let of_string s relation_delim elt_delim map_add map_empty key_of_string data_of_string = 
  ...
  List.fold_left 
    (fun m s -> 
       ...
       then 
	 let error = match kd_list with 
	     [] -> "Empty list" 
	   | _ -> Mstring.concat_list ":" kd_list ""
	 in
	 raise (Failure("Mmap.of_string: Ill-defined Key-Data -> "^error))
       else   
