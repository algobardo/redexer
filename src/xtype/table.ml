...
let to_stringlist col_delim t = 
  List.map 
    (fun x -> Mstring.concat_list col_delim (Array.to_list x) "")
    (Array.to_list t) 

let to_file ?col_delim:(col_delim="\t") t filename =
  let stringList = to_stringlist col_delim t
  in File.of_stringlist filename stringList

let print ?col_delim:(col_delim="\t") t =
  let stringList = to_stringlist col_delim t
  in List.iter print_endline stringList
